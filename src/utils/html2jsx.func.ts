...
let HTMLDOMPropertyConfig = require('react-dom/lib/HTMLDOMPropertyConfig');
let SVGDOMPropertyConfig = require('react-dom/lib/SVGDOMPropertyConfig');
 ...
 */
function eachObj(obj: any, iteratee: any, context?: any) {
    for (let key in obj) {
...
function mappingAttributesFromReactConfig(config: any) {
    eachObj(config.Properties, function (propname: any) {
 ...
 * @return {string}
 */
function jsxTagName(tagName: any) {
...
HTMLtoJSX.prototype = {
    ...
    _onlyOneTopLevel: function (containerEl) {
        ...
        for (let i = 0, count = containerEl.childNodes.length; i < count; i++) {
            let child = containerEl.childNodes[i];
    ...
    _beginVisitElement: function (node) {
        let tagName = jsxTagName(node.tagName);
        let attributes = [];
    ...
    _visitText: function (node) {
        let parentTag = node.parentNode && jsxTagName(node.parentNode.tagName);
